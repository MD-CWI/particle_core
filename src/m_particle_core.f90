...
module m_particle_core
  ...
  integer, public :: PC_num_neg_ions=0
  integer, public :: PC_num_pos_ions_by_e=0
  integer, public :: PC_num_pos_ions_by_g=0
  ...
  type, public :: PC_t
     ...
     procedure(p_to_r3_f), pointer, nopass   :: acc_function => null()
   ...
   contains
     ...
     procedure, non_overridable :: set_acc_function
...
contains  
  ...
  subroutine set_acc_function(self, pptr)
    class(PC_t), intent(inout) :: self
    procedure(p_to_r3_f) :: pptr
    self%acc_function => pptr
  end subroutine set_acc_function
    ...
    do
       ...
       if (cIx > 0) then
          ...
          case (CS_attach_t)
             ...
             PC_num_neg_ions = PC_num_neg_ions + nint(self%particles(ll)%w)
          case (CS_elastic_t)
          ...
          case (CS_ionize_t)
             call ionization_collision(self%particles(ll), part_out, &
                  n_part_out, self%colls(cIx), self%rng)
             PC_num_pos_ions_by_e = PC_num_pos_ions_by_e + &
                  nint(self%particles(ll)%w)
          case default
...
100 continue
  ...
  subroutine attach_collision(part_in, part_out, n_part_out, coll, rng)
    ...
    PC_num_neg_ions = PC_num_neg_ions + 1
  ...
  subroutine set_accel(self, accel_func_user)
    class(PC_t), intent(inout)      :: self
    procedure(p_to_r3_f), optional  :: accel_func_user
    procedure(p_to_r3_f), pointer   :: accel_func
    integer                         :: ll

    if(present(accel_func_user))then
      do ll = 1, self%n_part
         self%particles(ll)%a = accel_func_user(self%particles(ll))
      end do
    else
      do ll = 1, self%n_part
         self%particles(ll)%a = self%acc_function(self%particles(ll))
      end do
    endif
  ...
  subroutine correct_new_accel(self, dt, accel_func_user)
    use m_units_constants
    class(PC_t), intent(inout)      :: self
    real(dp), intent(IN)            :: dt
    procedure(p_to_r3_f), optional  :: accel_func_user
    ...
    if(present(accel_func_user))then
            accel_func=>accel_func_user
    else
            accel_func=>self%acc_function
    endif
